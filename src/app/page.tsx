...
const setInitialData = (addGoal: (goal: Goal) => void, addTask: (task: Task) => void) => {
  const goals: Goal[] = [
  ...
  const initialState = { goals, tasks };
  localStorage.setItem('goalFlowState', JSON.stringify(initialState));
...
export default function Home() {
  ...
  useEffect(() => {
    // One-time check to see if we should load the initial data.
    const hasBeenInitialized = localStorage.getItem('goalFlowInitialized');
    if (!hasBeenInitialized) {
      // We manually dispatch actions to avoid breaking the context's rules
      const dispatch = (context as any).dispatch;
      if (dispatch) {
          const initialState = {
              goals: [
                { id: 'goal-1', name: 'Desenvolvimento pessoal — Ser referência em dados e IA' },
